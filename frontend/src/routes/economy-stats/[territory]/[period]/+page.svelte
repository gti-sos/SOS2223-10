  ...
  async function updateEconomy() {
    ...
    try {
      ...
      if (status == 200) {
        ...
        color = "danger";
        checkMSG =
          "Error al actualizar los datos de economia, comprueba que hayas completado todos los campos y que los campos numericos son mayores que 0 y vuelve a intentarlo";
      }
    } catch (error) {
      console.error(error);
      msgVisible = true;
      color = "danger";
      checkMSG =
        "Hubo un error al intentar actualizar los datos de economia, intenta nuevamente más tarde";
    }
  }
</script>
    ...
    <table class="table table-bordered table-striped table-hover">
      ...
      <tbody>
        <tr>
          ...
          >
          <td
            ><input type="text" bind:value={tourist} class="form-control" /></td
          >

          <td
            ><Button
              on:click={updateEconomy}
