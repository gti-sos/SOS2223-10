  ...
  async function updateEmployment() {
    ...
    try {
      ...
      if (status == 200) {
        ...
        color = "danger";
        checkMSG =
          "Error al actualizar los datos de empleo, comprueba que hayas completado todos los campos y que los campos numericos son mayores que 0 y vuelve a intentarlo";
      }
    } catch (error) {
      console.error(error);
      msgVisible = true;
      color = "danger";
      checkMSG =
        "Hubo un error al intentar actualizar los datos de empleo, intenta nuevamente más tarde";
    }
  }
</script>
...
<body>
  <div class="page-container">
    ...
    <Alert {color} isOpen={msgVisible} toggle={() => (msgVisible = false)}>
      {#if checkMSG != ""}
        {checkMSG}
      {/if}
    </Alert>
    ...
    <table class="table table-bordered table-striped table-hover">
      <thead>
        <tr>
          ...
          <th>Periodo</th>
          <th width="89px">Poblacion mayor de 16</th>
      ...
      <tbody>
        <tr>
          ...
          <td
            ><input
              type="text"
              bind:value={population_over_16_years}
              class="form-control"
            /></td
          >
          <td
            ><input
              type="text"
              bind:value={activity_men_percentage}
              class="form-control"
            /></td
          >
          <td
            ><input
              type="text"
              bind:value={activity_women_percentage}
              class="form-control"
            /></td
          >
          ...
          <td
            ><Button
              on:click={updateEmployment}
              class="bg-warning font-weight-bold">Editar</Button
            ></td
          >
        </tr>
      </tbody>
