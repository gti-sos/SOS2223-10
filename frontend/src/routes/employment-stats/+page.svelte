<main>
    <h1>
        Bienvenido a la API employment_stats 
    </h1>
    ...
    // @ts-nocheck
        ...
        async function createEmployment (){
            ...
            const existingData = insertedData.find(data => 
                data.period === newperiod && data.province === newProvince && data.population_over_16_years === newPopulation_over_16_years
...
<body>
    ...
    <div class="botones">
        ...
        <div class="button-toolbar">
            <button class="button" on:click={loadData}>Datos Empleo</button>
        </div>
        {#if mensajeUsuario !=""}
...
<form on:submit|preventDefault={searchEmployment} class="p-3 border rounded">
    <div class="form-group">
      <label for="province" class="font-weight-bold">Provincia:</label>
      <input type="text" class="form-control form-control-sm" id="province" bind:value={newProvince} required>
    </div>
    <div class="form-group">
      <label for="period" class="font-weight-bold">Period:</label>
      <input type="number" class="form-control form-control-sm" id="period" bind:value={newPeriod} required>
    </div>
    <Button color="primary" type="submit" class="mt-3">Buscar</Button>
  </form>
<div>
  <button class="button" on:click={getEmployment}>Obtener Datos Empleo</button>
</div>
    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                <td><button class="button" on:click={createEmployment}>Crear</button></td>
            </tr>
            ...
            <tr>
              ...
              <td>
                <Button color="info" class="btn-sm">
                  <a href='employment-stats/{Employment.province}/{Employment.period}' class="text-white text-decoration-none">Editar</a>
                </Button>
              </td>
              <td>
                <Button color="danger" class="btn-sm" on:click={deleteEmployment(Employment.province,Employment.period)}>
                  Borrar
                </Button>
              </td>
            </tr>
          ...
          <div class="mt-3">
            <Button color="danger" on:click={deleteEmploymentAll} class="btn-sm">
              Borrado de Datos
            </Button>
          </div>
        </tbody>
